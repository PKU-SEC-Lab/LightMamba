...
class EXP_QUANT_Blackbox extends BlackBox {
  val top_name: String = "EXP_QUANT"
...
class EXP_QUANT extends Component {
  val top_name: String = "EXP_QUANT"
  setDefinitionName(top_name + "_wrapper")
  private val black_box = new EXP_QUANT_Blackbox
  val io = new Bundle {
...
object simulate_exp_quant extends App {
  redirect_std("EXP_QUANT.log")
  ...
  spinalConfig.generateVerilog(new EXP_QUANT)
  ...
  SimConfig
    ...
    .doSimUntilVoid { dut =>
      ...
      // @formatter:off
      // ref arrays
      val exp_in = read_multi_int64_files(format_str, L, "/dA_before_exp_layer", T * C)
      val exp_out_q = read_multi_int64_files(format_str, L, "/dA_exp_q_layer", T * C)
      val exp_out_s  = read_multi_int64_files(format_str, L, "/dA_exp_s_layer", T * CT)
      // dut arrays
      val DUT_EXP_Q  = Array.ofDim[Long](L, TP * C)
      val DUT_EXP_S  = Array.ofDim[Long](L, TP * CT)
      ...
      Array(
        ...
        // @formatter:off
        fork {
          for (l <- 0 until L) array2stream(dut.io.i_stream, dut.clockDomain, exp_in(l), 1, 1, TP, 1, C,  CP, info="i stream", verbose=true, WAIT=320)
        },
        fork {
          for (l <- 0 until L) stream2array(dut.io.o_stream,  dut.clockDomain, exp_out_q(l), DUT_EXP_Q(l),  1, TP, 1, C,  CP, info="o stream")
        },
        fork {
          for (l <- 0 until L) stream2array(dut.io.o_s_stream,  dut.clockDomain, exp_out_s(l), DUT_EXP_S(l),  1, TP, 1, CT, 1, info="o_s stream")
        }
        // @formatter:on
      ).foreach(_.join())

      // calculate max of XM_S
      val max_XM_S = DUT_EXP_Q.flatten.max
