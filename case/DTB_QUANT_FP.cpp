...
#include "../src/dtB_fp.h"
#include "../src/quantizer_fp.h"
#include <pthread.h>

/** @brief Number of test layers */
constexpr int TEST_L = 1;
...
QUANTIZER<X_T, A_T, float, 1, T, TP, C * N, CP, G> quantizer_inst;
 ...
 * @param o_s_stream Output stream of floating-point scale data with single element vectors.
 */
void top(
    ...
    hls::stream<hls::vector<float,  1> >& o_s_stream    
){
...
void test_layer(int l){
    ...
    array2stream<int64_t, float, 1, 1, T, TP, CT, 1>(REF_DT_S, dt_s_stream, "DT_S", true);
    array2stream<int64_t, A_T, CT, 1, T, TP, N, NP>(REF_B_Q, B_stream, "B_Q", true);
    array2stream<int64_t, float, CT, 1, T, TP, NT2, 1>(REF_B_S, B_s_stream, "B_S", true);
...
// Read output streams into arrays
    ...
    stream2array<int64_t, float, 1, T, TP, S_T,  1>(o_s_stream, DUT_DTB_S, "DTBs", true);
