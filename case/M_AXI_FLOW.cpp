/**
 * @brief Implementation of weight processing and data distribution for neural network layers.
 *
 * This file contains functions for producing, distributing, and composing weights and data streams
 * for a neural network accelerator, handling memory access, data packing, and stream operations.
 */
...
/**
 * @brief Burst length for memory transfers.
 */
constexpr int BURST_LEN         = 256;
/**
 * @brief Maximum data width for AXI interface.
 */
...
/**
 * @brief Output channels for first convolution.
 */
constexpr int CO1  = 10576;
/**
 * @brief Input channels for second convolution.
 */
constexpr int CI2  = 5120;
/**
 * @brief Output channels for second convolution.
 */
constexpr int CO2  = 2560;

/**
 * @brief Combined channel count.
 */
...
constexpr int TP = 1;
/**
 * @brief Channels per processing element.
 */
constexpr int CP = G;
/**
 * @brief Total number of quantized weights.
 */
constexpr int NUM_Q = CI1*CO1 + CI2*CO2;//40181760
/**
 * @brief Total number of scaling factors.
 */
constexpr int NUM_S = NUM_Q / GW;        // 313920

/**
 * @brief Number of quantized weights for convolution.
 */
constexpr int C_NUM_Q = 4*CC;//21504
/**
 * @brief Number of scaling factors for convolution.
 */
constexpr int C_NUM_S = C_NUM_Q / G;//2688

/**
 * @brief Number of quantized weights for hidden layer.
 */
constexpr int H_NUM_Q = CI2*128;//655360
/**
 * @brief Number of scaling factors for hidden layer.
 */
...
/**
 * @brief Produces weight and convolution data streams from memory.
 *
 ...
 * @param conv2_stream Output stream for merged convolution data部分
*/
void conv_merge(
    hls::stream<maxi_t>& cq2_queue,
    hls::stream<maxi_t>& cs2_queue,
 ...
 *
 * Coordinates the dataflow for weight production, distribution, composition, and decomposition
 * across multiple layers, interfacing with the accelerator.
 *
 * @param l_begin Starting layer index.
 * @param l_close Ending layer index.
 * @param mem_stream Input stream for weight data.
 * @param conv_stream Input stream for convolution data.
 * @param conv2_stream Output stream for convolution output.
 * @param ht_stream Input stream for hidden layer data.
 * @param ht2_stream Output stream for hidden layer output.
 * @param wq_stream Output stream for quantized weights.
 * * @param ws1_stream Output stream for first weight scaling factors.
 * @param ws2_stream Output stream for second weight scaling factors.
 * @param cq_stream Output stream for convolution quantization data.
 * @param cs_stream Output stream for convolution scaling factors.
 * @芒param cq2_stream Output stream for convolution quantization output.
 * @param cs2_stream Output stream for convolution scaling output.
 * @param hq_stream Output stream for hidden layer quantization data.
 * @param hs_stream Output stream for hidden layer scaling factors.
 * @param hq2_stream Output stream for hidden layer quantization output.
 * @param hs2_stream Output stream for hidden layer scaling output.
 */
void top(
    int l_begin,
    int l_close,
 ...
 *
 * Initializes streams, writes test data, calls the top function, and verifies stream emptiness.
 *
...
int main(){
    ...
    for(int l=0;l<L;l++) {
        for(int n=0;n<C_CYCS;n++) {
            maxi_t vec=conv2_stream.read();
        }
        for(int n=0;n<H_CYCS;n++) {
            maxi_t vec=ht2_stream.read();
        }
    }
    for(int l=0;l<L;l++) {
