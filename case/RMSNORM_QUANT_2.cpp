/**
 * This module tests the RMS normalization operation with divided weights followed by quantization by
 * loading reference data, creating input streams, executing the RMSNORM and QUANTIZER operations, and
 * comparing the output against reference data. The implementation is optimized for HLS with dataflow
 * and stream interfaces.
 */
#include "../src/common.h"
#include "../src/rmsnorm2_divide_lnw.h"
 ...
 *
 * Executes RMS normalization with divided weights followed by quantization on the input stream,
 * producing quantized output and scale streams.
 *
...
void top(
    ...
    /**< Execute RMS normalization with divided weights */
 ...
 *
 * Loads reference data, creates input streams, runs the RMSNORM and QUANTIZER operations, and compares
...
void test_layer(int l){
    ...
    // read refs, both MHA and MLP
     ...
     // Read reference data
    auto INPUT_X                  = read_tensor<int64_t>(file_path + "/yz_layer"+file_path_suffix);
    auto OUTPUT_XLN_Q             = read_tensor<int64_t>(file_path + "/rms2_q_layer"+file_path_suffix);
    auto OUTPUT_XLN_S             = read_tensor<int64_t>(file_path + "/rms2_s_layer"+file_path_suffix);
