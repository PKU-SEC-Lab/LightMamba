#ifndef __INT_CONV_DIVIDE_H__
#define __INT_CONV_DIVIDE_H__
                ...
                for(int d=0;d<D+2;d++) {
                    ...
                    if(d<1) {
                        for(int cp=0;cp<CP;cp++) {
                            ...
                            WSCALE_T_ conv_b_s;
                            if(r<1) {
                                conv_b=XD_T(conv_bias[l][ct * CP + cp]);
                                conv_b_s=conv_bias_s[l][ct];
                            }
                            else {
                                conv_b=XD_T(conv_bias2[l][ct * CP + cp]);
                                conv_b_s=conv_bias_s[l][ct+CT2];
                            }
                            o_vec[cp] = conv_b << conv_b_s;
                        }
                    }
                    else if(d<D+1) {
                        ...
                        ap_uint<5> scale;
                        if(r<1) {
                            scale = s_vec[0] + conv_weight_s1[l][ct][d-1];
                        }
                        else {
                            scale = s_vec[0] + conv_weight_s2[l][ct][d-1];
                        }
                        for(int cp=0;cp<CP;cp++) {
                            #pragma HLS unroll
                            WC_T weight;
                            if(d<2) {
                                if(r<1)
                                    weight=conv_weight1[l][ct*CP+cp];
                                else
                                    weight=conv_weight2[l][ct*CP+cp];
                            }
                            else if(d<3) {
                                if(r<1)
                                    weight=conv_weight3[l][ct*CP+cp];
                                else
                                    weight=conv_weight4[l][ct*CP+cp];
                            }
                            else if(d<4) {
                                if(r<1)
                                    weight=conv_weight5[l][ct*CP+cp];
                                else
                                    weight=conv_weight6[l][ct*CP+cp];
                            }
                            else {
                                if(r<1)
                                    weight=conv_weight7[l][ct*CP+cp];
                                else
                                    weight=conv_weight8[l][ct*CP+cp];
                            }
                            auto mul_res = i_vec[cp] * weight;
