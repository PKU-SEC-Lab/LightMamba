/**
 * @file DTB_fp.h
 * @brief Header file for the DTB class implementing a dot-product transformation operation with floating-point scaling.
 *
 * This class performs a dot-product transformation (DTB) operation for the Llama model, processing input streams with
 * floating-point scaling factors and producing an output stream. The implementation is optimized for HLS with pipelining,
 * dataflow, and unrolling directives.
 */
#ifndef __INT_DTB_FP_H__
#define __INT_DTB_FP_H__
...
/**
 * @class DTB
 * @brief Template class for computing dot-product transformation operations with floating-point scaling.
 *
 * Processes input streams of data and scaling factors, performs multiplication with floating-point scaling,
...
/** @brief Number of channel tiles */
     ...
     *
     * Reads input streams for data and scaling factors, performs multiplication with floating-point scaling,
     * and writes the results to the output stream.
     *
     * @param dt_stream Input stream of data with CP parallelism.
     * @param dt_s_stream Input stream of floating-point scale data with single element vectors.
     * @param B_stream Input stream of data with CP parallelism.
     * @param B_s_stream Input stream of floating-point scale data with single element vectors.
     * @param o_stream Output stream of computed results with CP parallelism.
     */
    void do_dtB(
        hls::stream<hls::vector<A_T,      CP> >& dt_stream,
        hls::stream<hls::vector<float, 1 > >& dt_s_stream,
        hls::stream<hls::vector<A_T,      CP> >& B_stream,
        hls::stream<hls::vector<float, 1 > >& B_s_stream,
        hls::stream<hls::vector<X_T,      CP> >& o_stream
    ) {
        #pragma HLS dataflow

        hls::vector<A_T, CP> dt_vec;
        hls::vector<float, 1> dt_s_vec;
        hls::vector<A_T, CP> B_vec;
        hls::vector<float, 1> B_s_vec;
            ...
            for(int nt=0;nt<NT2+1;nt++) {
                ...
                for(int np=0;np<NP;np++) {
                    ...
                    if(nt<NT2) {
                        ...
                        float scale = dt_s_vec[0] * B_s_vec[0];
                        #pragma HLS bind_op variable=scale op=fmul impl=dsp
                        // Perform multiplication and scaling
                        for(int cp=0;cp<CP;cp++) {
                            #pragma HLS unroll
                            auto mul_res=dt_vec[cp]*B_vec[np];
                            #pragma HLS bind_op variable=mul_res op=mul impl=dsp
                            auto mul_res2=XD_T(mul_res) * scale;
                            #pragma HLS bind_op variable=mul_res2 op=fmul impl=dsp
                            o_vec[cp]=mul_res2;
                        } 
