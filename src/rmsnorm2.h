/**
 * @file rmsnorm2.h
 * @brief Header file for the RMSNORM class implementing RMS normalization 
 ...
 * unrolling, and array reshaping directives. This is a second variant tailored for specific model requirements.
 */
#ifndef __INT_RMSNORM2_H__
#define __INT_RMSNORM2_H__
...
/**
 * @class RMSNORM
 * @brief Template class for performing RMS normalization in the Llama model (second variant).
 *
...
public:
    ...
    RMSNORM(const init_t lnw_init[L*C]){
        for(int l=0; l<L; ++l){
            for(int c=0; c<C; ++c){
                LNW[l][c] = lnw_init[l*C + c];  /**< Initialize weight matrix */
            }
        }
    }
    ...
    void do_rmsnorm(int l, hls::stream<hls::vector<X_T, TP * CP> > &i_stream, hls::stream<hls::vector<XLN_T, TP * CP> > &o_stream){
       ...
       for(int tt = 0; tt < TT; ++tt) { /**< Iterate over time tiles */
            ...
            for(int ct = 0; ct < CT; ++ct) { /**< Iterate over channel tiles */
                ...
                hls::vector<X_T, TP * CP> i_vec = i_stream.read(); /**< Read input vector */
                if(ct == 0) { /**< Reset X_POW2SUM at the start of each time tile */
                    for(int tp = 0; tp < TP; ++tp) {
                        #pragma HLS unroll /**< Unroll loop for parallel processing */
                        X_POW2SUM[tp] = 0; /**< Initialize sum of squares */
                    }
                }
                for(int tp = 0; tp < TP; ++tp) { /**< Iterate over time parallelism */
                    for(int cp = 0; cp < CP; ++cp) { /**< Iterate over channel parallelism */
                        ...
                        X_POW2SUM[tp] = X_POW2SUM[tp] + i_vec[tp * CP + cp] * i_vec[tp * CP + cp]; /**< Accumulate sum of squares */
                    }
                }
            }
            ...
            for(int ct = 0; ct < CT; ++ct) { /**< Iterate over channel tiles */
                #pragma HLS pipeline II=1 /**< Enable pipelining with initiation interval of 1 */
                hls::vector<XLN_T, TP * CP> o_vec; /**< Output vector for normalized data */
                for(int tp = 0; tp < TP; ++tp) { /**< Iterate over time parallelism */
                    for(int cp = 0; cp < CP; ++cp) { /**< Iterate over channel parallelism */
                        #pragma HLS unroll /**< Unroll loop for parallel processing */
                        auto X_MUL_X_RSQRT = (X_BUF[tp][ct * CP + cp] * X_RSQRT[tp]) >> RSQRT_TRUNC_MUL1; /**< Compute scaled input */
                        o_vec[tp * CP + cp] = (X_MUL_X_RSQRT * LNW[l][ct * CP + cp]) >> RSQRT_TRUNC_MUL2; /**< Apply weight and truncate */
                    }
                }
